...
def _conf_score_from_p(pvals):
    """Certainty score for charts: -log10(p). Higher = more certain."""
...
def _fallback_reliability_band(df, great=300, good=100, directional=50):
    if "Reliability_Band" in df.columns:
        return df["Reliability_Band"].astype(str)
    ...
    elif "Reliability_N" in df.columns:
        n = df["Reliability_N"].astype(float).to_numpy()
    else:
...
def _fallback_clarity_band(df, clear_p=0.05, directional_p=0.10, good_threshold=100):
    if "Clarity_Band" in df.columns:
        return df["Clarity_Band"].astype(str)

    p = df["P_Value"].astype(float) if "P_Value" in df.columns else pd.Series([np.nan] * len(df), index=df.index)

    if "Reliability_N" in df.columns:
        min_n = df["Reliability_N"].astype(float)
    elif "Control Sample" in df.columns and "Exposed Sample" in df.columns:
        min_n = np.minimum(
            df["Control Sample"].astype(float).to_numpy(),
            df["Exposed Sample"].astype(float).to_numpy(),
        )
        min_n = pd.Series(min_n, index=df.index)
    else:
        min_n = pd.Series([np.nan] * len(df), index=df.index)

    out = []
    for pv, mn in zip(p.tolist(), min_n.tolist()):
        if not (pv == pv):
            out.append("Unclear")
            continue
...
def _quadrant_from_effect(diff_pp, certainty, y_thr=1.301, x_thr=0.0):
    # x-axis = effect (pp), y-axis = certainty
...
def executive_impact_matrix(df):
    ...
    # IMPORTANT: keep x/y as pandas Series so .loc works
    x = (d["Diff_PctPts"].astype(float) if "Diff_PctPts" in d.columns else d["Lift_Pct"].astype(float))
    y_arr = _conf_score_from_p(d["P_Value"].astype(float)) if "P_Value" in d.columns else np.zeros(len(d))
    y = pd.Series(y_arr, index=d.index, dtype=float)
    ...
    # Hover payload helpers
    def col_float(name):
        if name in d.columns:
            return d[name].astype(float)
        return pd.Series([np.nan] * len(d), index=d.index, dtype=float)

    control_pct = col_float("Control_Pct")
    exposed_pct = col_float("Exposed_Pct")
    diff_pp = col_float("Diff_PctPts")
    lift_pct = col_float("Lift_Pct")
    p_val = col_float("P_Value")
    ...
    fig.update_layout(
        ...
        xaxis_title="Effect (pp) — right is positive, left is negative",
...
def executive_forest_plot(df, top_n=25):
    """Effect sizes with confidence intervals (top N by absolute gap)."""
    d = _ensure_bands(df).copy()
    ...
    fig.update_layout(
        ...
        xaxis_title="Difference (pp) — exposed minus control",
        yaxis_title="",
        margin=dict(l=20, r=20, t=55, b=20),
